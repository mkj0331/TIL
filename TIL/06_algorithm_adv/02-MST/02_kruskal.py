...
def mst_kruskal(vertices, edges):
    '''
        1. 가중치를 오름차순 정렬한다.(우선순위 큐 느낌)
        2. 그 순서대로 간선들을 선택하는데
        3. 2에서 선택한 간선의 시작, 종료 노드가 같은 대표자가 아니어야 한다.(사이클 방지)
        4. 2와 3을 선택된 간선이 n-1개가 될 때까지 반복한다.
    '''
    mst = []    # 최소 신장 트리를 구성하는 간선 정보를 담을 리스트
    # 간선 정보의 구성 [[시작노드, 종료노드, 가중치], ..]
    edges.sort(key=lambda x: x[2])  # 가중치를 기준으로 오름차순 정렬한다.
    # print(edges)
    ...
    for edge in edges:
        ...
        '''
            ...
            이제 우리가 할 일은
            s와 e가 같은 집합에 속해있는지(대표자가 같은지) 확인 하고,
            만약 같은 집합에 속해있지 않은 노드들이라면 합칠 수 있음!
        '''
        if ds.find_set(s) != ds.find_set(e):
            ds.union(s, e)
            mst.append(edge)

    return mst   # [[1, 2, 1], [1, 3, 2]]
